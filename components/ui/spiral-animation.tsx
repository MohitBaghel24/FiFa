...
class AnimationController {
    ...
    public render() {
        ...
        ctx.fillStyle = 'rgba(0, 255, 135, 0.8)' // Changed star color to accent color
    ...
    private drawTrail(t1: number) {
        for (let i = 0; i < this.trailLength; i++) {
            ...
            this.ctx.fillStyle = 'rgba(0, 255, 135, 0.5)' // Changed trail color to accent
            this.ctx.lineWidth = sw
    ...
    public setSpeed(multiplier: number) {
        gsap.to(this.timeline, { timeScale: multiplier, duration: 1.5, ease: "power2.inOut" })
    }
...
export function SpiralAnimation({ isFast = false }: { isFast?: boolean }) {
    ...
    useEffect(() => {
        if (animationRef.current) {
            animationRef.current.setSpeed(isFast ? 10 : 1);
        }
    }, [isFast])
    ...
    return (
        <div ref={containerRef} className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none opacity-40">
