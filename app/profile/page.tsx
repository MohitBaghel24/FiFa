...
export default function ProfilePage() {
  ...
  const activityGrid = Array.from({ length: 70 }, (_, i) => {
    ...
    let opacity = 'bg-bg-primary border border-border-color'; // 0
    if (val > 0.85) opacity = 'bg-accent opacity-100 shadow-[0_0_5px_rgba(255,255,255,0.1)]';
    ...
    <div className="flex flex-col min-h-[calc(100vh-64px)] w-full bg-bg-primary pb-20 relative">
      ...
      <div className="relative w-full h-[280px] bg-bg-secondary border-b border-border-color">
        ...
        <div className="absolute top-6 right-6 flex items-center gap-3 z-10">
          ...
          <button className="btn-primary !px-6 !py-2 !h-10 text-xs shadow-[0_0_15px_rgba(255,255,255,0.15)]">
            FOLLOW
          </button>
        </div>
      ...
      <div className="max-w-6xl mx-auto w-full px-4 md:px-12 mt-16 md:mt-12 flex flex-col gap-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mt-4 md:mt-0">
          {stats.map((stat, i) => (
            <div key={i} className="bg-bg-card border border-border-color rounded-xl p-5 md:p-6 flex flex-col gap-3 hover:border-accent/50 transition-colors group relative overflow-hidden shadow-lg">
        ...
        {/* BADGE COLLECTION */}
        <div className="flex flex-col gap-5 bg-bg-secondary border border-border-color p-6 rounded-xl">
          ...
          <div className="flex flex-wrap gap-4 py-2">
            {badges.map((badge) => (
              <div
                key={badge.id}
                className={`relative w-[72px] h-[80px] flex items-center justify-center transition-all duration-300 ${badge.earned
                  ? 'hover:scale-110 cursor-pointer z-10'
                  : 'opacity-40 cursor-not-allowed grayscale'
                  }`}
              >
                {/* Outer Hexagon (Border) */}
                <div className={`absolute inset-0 ${badge.earned ? 'bg-gold' : 'bg-border-color'}`}
                  style={{ clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)' }}></div>

                {/* Inner Hexagon (Background) */}
                <div className={`absolute inset-[2px] ${badge.earned ? 'bg-bg-card' : 'bg-bg-primary'}`}
                  style={{ clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)' }}></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            ...
            {/* Recent Predictions */}
            <div className="bg-bg-card border border-border-color rounded-xl overflow-hidden shadow-lg">
              <div className="px-5 py-4 border-b border-border-color bg-bg-secondary">
              ...
              <div className="flex flex-col">
                {recentPredictions.map((pred) => (
                  <div key={pred.id} className="flex items-center justify-between px-5 py-4 border-b border-border-color last:border-b-0 hover:bg-bg-card-hover transition-colors">
                    <div className="flex items-center gap-4">
                      {pred.result ? (
                        <CheckCircle2 size={20} className="text-accent drop-shadow-[0_0_5px_rgba(255,255,255,0.3)]" />
                      ) : (
            ...
            {/* Community Posts */}
            <div className="bg-bg-card border border-border-color rounded-xl overflow-hidden shadow-lg">
              <div className="px-5 py-4 border-b border-border-color bg-bg-secondary flex justify-between items-center">
                <h3 className="text-xs font-bold text-text-secondary uppercase tracking-widest flex items-center gap-2">
              ...
              <div className="flex flex-col p-5 gap-6">
                {communityPosts.map((post) => (
                  <div key={post.id} className="flex gap-4 pb-5 border-b border-border-color last:border-b-0 last:pb-0">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            {/* Activity */}
            <div className="bg-bg-card border border-border-color rounded-xl overflow-hidden p-6 flex flex-col gap-5 shadow-lg">
              <h3 className="text-xs font-bold text-text-secondary uppercase tracking-widest flex items-center gap-2">
              ...
              <div className="flex justify-between items-center text-[9px] text-text-muted font-bold tracking-widest uppercase mt-2">
                ...
                <div className="flex gap-[3px]">
                  <div className="w-[12px] h-[12px] rounded-[2px] bg-bg-primary border border-border-color"></div>
                  ...
                  <div className="w-[12px] h-[12px] rounded-[2px] bg-accent opacity-100 shadow-[0_0_5px_rgba(255,255,255,0.1)]"></div>
                </div>
                <span>MORE</span>
              </div>
            </div>
